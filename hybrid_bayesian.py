...
def cf_prob_func(p):
   par =  users_sim_act_user
   prob = 0
     
   for usr in par:
       rel_sim = sim[usr]/tot_sim
...
##################### add edges from the feature nodes to the item nodes ##################################
  ...
  ################### find the users similar to the active user and add the nodes to the BN  ####################
  sim = np.zeros(len(user_rat_mat))
  ...
  for usr in range(0,len(user_rat_mat)):
      ...
      if( (den1 == 0) or (den2 == 0)):
          ...
          sim[usr] = num/math.sqrt(den1*den2)
          sim[usr] = sim[usr] * (len(rat_both)/len(item_rat_act_user))
